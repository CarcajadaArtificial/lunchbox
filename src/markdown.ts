...
import { emojify } from 'https://deno.land/x/emoji@0.3.0/mod.ts';
import * as Marked from 'https://esm.sh/marked@5.1.0/';
import { mangle } from 'https://esm.sh/marked-mangle@v1.1.0/';
import { gfmHeadingId } from 'https://esm.sh/marked-gfm-heading-id@3.0.4/';
import { default as Prism } from 'https://esm.sh/prismjs@1.29.0';
import { default as sanitizeHtml } from 'https://esm.sh/sanitize-html@2.8.1?target=esnext';
import { escape as htmlEscape } from 'https://esm.sh/he@1.2.0';
import { default as katex } from 'https://esm.sh/katex@0.16.8/dist/katex.mjs';
import { KATEX_CLASSES } from './katex.ts';
export { KATEX_CSS } from './katex.ts';
export { Marked };
...
class Renderer extends Marked.Renderer {
  ...
  code(code: string, language?: string) {
    ...
    if (grammar === undefined) {
      return `<pre><code ${
        customClass(this.customClasses.code, 'notranslate')
      }>${
        htmlEscape(
          code,
        )
  ...
  link(href: string, title: string | null, text: string) {
    ...
    return `<a ${
      customClass(this.customClasses.link)
    } href="${href}"${titleAttr} rel="noopener noreferrer">${text}</a>`;
  }

  blockquote(quote: string) {
    return `<blockquote ${
      customClass(this.customClasses.blockquote)
    }>\n${quote}</blockquote>\n`;
  }
  ...
  tablerow(content: string) {
    return `<tr ${
      customClass(this.customClasses.tablerow)
    }>\n${content}</tr>\n`;
  }
